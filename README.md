...
> automated KYC, and uses stubbed SMS & mobile-money providers (see [Swapping the stubs](#swapping-the-stubs)).
...
| Area | Status | Details |
|---|---|---|
| OTP phone login | ✅ Implemented | Real 6-digit codes, 10-min TTL, single-use, 5-attempt brute-force cap. *Delivery* is stubbed (printed to the server console). |
| Passwordless JWT sessions | ✅ Implemented | 30-day signed tokens; `JWT_SECRET` must be changed in production. |
...
| SMS / OTP delivery | 🟡 **Stub** | `server/src/providers/sms.js` logs codes to the console; swap in a real gateway (Twilio, Africa's Talking, Vonage…). |
...
| ID verification / KYC | 🟡 **Partial** | Documents are uploaded and stored for **manual staff review**; no automated KYC. |
...
| Email / notifications | ❌ Not implemented | Only the SMS-stub console output exists. |
...
│  │  ├─ providers/
│  │  │  ├─ sms.js         # STUB OTP delivery (console)
│  │  │  └─ payments.js    # STUB mobile-money escrow (Telebirr-style)
│  │  ├─ services/         # deals lifecycle, disputes, reputation engine
...
- **SMS/OTP** (`server/src/providers/sms.js`): replace `sendOtp()`/`sendMessage()` with a real gateway call
  (Africa's Talking, Twilio, Vonage, …). The auth route is unchanged.
...
- **ID verification**: documents are uploaded and stored for **manual review** (staff flips the status via the
  API). Swap in automated KYC later without touching the data model.
...
- Automated ID verification (manual review only)
